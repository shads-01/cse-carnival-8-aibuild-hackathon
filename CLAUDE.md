...
- [x] Phase 4 — Agent backend (tools + tool-use loop) — `tools.ts` now calls the real `services/*.ts` (Supabase-backed) directly, `stubData.ts`/`types.local.ts` deleted; live-smoke-tested against real Gemini + Supabase (2026-09-04); see `ARCHITECTURE.md`'s Build phases table
