...
- [ ] Fill out `.env.example` with final key names (incl. `GEMINI_API_KEY` for Hrittika's track)

## Your build
- [ ] `db/schema.sql` — 7 tables + FKs: `schedules`, `rooms`, `bookings`, `events`, `event_registrations`, `announcements`, `assignments`
