...
- [x] ~~`server/src/routes/v1/schedule.routes.ts`, `room.routes.ts`, `event.routes.ts`, `announcement.routes.ts`, `assignment.routes.ts` + matching `server/src/controllers/*.controller.ts` — thin routers + controllers over services, full CRUD on all 5~~
...
- [x] ~~Curl/Postman every endpoint against seeded data.~~ (Automated Vitest integration test suite passing 35/35 tests)
