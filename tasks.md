...
- [ ] `db/schema.sql` — create the 7 tables + foreign keys in Supabase
- [ ] `db/seed.ts` — load `data/*.json` into Supabase, transforming embedded
      `bookings`/`registrations` arrays into the join tables; safe to re-run (upsert on `id`)
- [ ] `services/scheduleService.ts`
- [ ] `services/roomService.ts` (+ `findAvailable`, `book`, `cancelBooking`)
- [ ] `services/eventService.ts` (+ `register`, `cancelRegistration`)
- [ ] `services/announcementService.ts`
- [ ] `services/assignmentService.ts`
- [ ] `routes/schedules.ts`, `rooms.ts`, `events.ts`, `announcements.ts`,
...
- [ ] Express app scaffold: `index.ts`, CORS, JSON body parsing, error middleware
